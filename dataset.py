...
def random_shift_events(events, max_shift=20, resolution=(346, 260), bounding_box=None):
    H, W = resolution
...
def random_flip_events_along_x(events, resolution=(346, 260), p=0.5, bounding_box=None):
    H, W = resolution
    flipped = False
...
class PKU_DDD17:
        def __init__(self, root, object_classes, height, width, nr_events_window=-1, augmentation=False, mode ='training',
        event_representation='histogram', shuffle=False):
         if mode == 'training':
            mode = 'training'
         elif mode == 'testing':
            mode = 'test'
         if mode == 'validation':
            mode = 'validation'
         root = '/home/xxyfehlen501602/workspace/rpg_asynet/'
         self.files =[]
         file_dir = []
         object_classes = [
         ...
         file_dir.append(listdir(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/dvs_events/" ,mode)))
         for dir in enumerate (file_dir[0]):
             new_files = [os.path.join(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/dvs_events/" ,mode, str(dir[1])), x) for x in os.listdir(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/dvs_events/" ,mode, str(dir[1])))]
             self.files += new_files
         self.object_classes = object_classes
         self.root = root
         self.width = width
         ...
         self.labels_path = []
         labels_dir = []
         labels_dir.append(listdir(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/labels/" ,mode)))
         for dir in enumerate (labels_dir[0]):
             new_labels = [os.path.join(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/labels/" ,mode, str(dir[1])), x) for x in os.listdir(os.path.join(root, "data/PKU-DDD17-CAR  Dataset/labels/" ,mode, str(dir[1])))]
             self.labels_path += new_labels
        
         for label_path in enumerate (self.labels_path):   
             with open(label_path[1]) as in_file:
             ...
             for obj in root.iter('object'):
                ...
                if label not in self.object_classes or int(
                    difficult) == 1:  # exclude difficult or unlisted classes
                    continue
                xml_box = obj.find('bndbox')
                # Required Information ['x', 'y', 'w', 'h', 'class_id'](x, y) is top left point\
                bbox = (int(xml_box.find('xmin').text), int(xml_box.find('ymax').text), int(xml_box.find('xmax').text)-int(xml_box.find('xmin').text),
                int(xml_box.find('ymax').text)-int(xml_box.find('ymin').text), int(object_classes.index(label)))
                #boxes.extend(bbox)  # return transpose so last dimension is variable length
                boxes.append(bbox)
        ...
        def __getitem__(self, idx):
         """
        returns events and label, loading events from pkl files
        :param idx:
        :return: x,y,t,p,  label
        """
         label = self.labels[idx]
         filename = self.files[idx]
         #events = np.load(filename).astype(np.float32)
         #for filename in enumerate (self.files):
         # with open (str(filename[1]), 'rb') as pickle_file:
         ...
         return events, bounding_box, histogram
        ...
        def generate_event_histogram(events, shape):
         """
         Events: N x 4, where cols are t, y, x, polarity, and polarity is in {0,1}. x and y correspond to image
         coordinates u and v.
         """
         H, W = shape
